...
import { required, minLength } from '../../services/validationService.js';
...
const LoginPage = () => {
  ...
  const {
    ...
    initialValues: {
      identifier: '', // Changed from email to identifier (can be username or email)
      password: '',
    },
    validations: {
          ...
          <form onSubmit={handleSubmit} className="login-form">
            <div className="form-group">
              ...
              <input
                ...
                className={`form-control ${touched.identifier && errors.identifier?.length > 0 ? 'is-invalid' : ''}`}
                disabled={state.isLoading}
              />
              {touched.identifier && errors.identifier?.length > 0 && (
                <div className="invalid-feedback">{errors.identifier[0]}</div>
              )}
            </div>

...
const toUserRoleEnum = (role): UserRole => {
  switch (role) {
    case "admin":
...
const AdminDashboard: React.FC = () => {
  ...
  const [users, setUsers] = useState([
    { id, username: "admin", email: "admin@example.com", role: "admin", status: "active" },
    { id, username: "teacher1", email: "teacher1@example.com", role: "teacher", status: "active" },
    { id, username: "student1", email: "student1@example.com", role: "student", status: "active" },
  ]);

  const [surveys, setSurveys] = useState([
    { id, title: "Course Evaluation Q1 2025", creator: "teacher1", responses, status: "active" },
    { id, title: "Teaching Quality Assessment", creator: "teacher1", responses, status: "draft" },
  ]);
  ...
  const handleDeleteUser = useCallback((userId) => setUsers((prev) => prev.filter((u) => u.id !== userId)), []);
  const handleDeleteSurvey = useCallback((surveyId) => setSurveys((prev) => prev.filter((s) => s.id !== surveyId)), []);
